  ...
  private formatOracleSchema(rows: any[]): string {
    ...
    return Array.from(tableMap.entries())
      .map(([table, columns]) => {
        return `Table ${table} {\n${columns
          .map(
            (col) => `  ${col.COLUMN_NAME} ${col.DATA_TYPE}(${col.DATA_LENGTH})`
          )
          .join("\n")}\n}`;
      })
      .join("\n\n");
  ...
  private async generateQuery(
    ...
    try {
      const prompt = `
            ...
            Convert this natural language query to SQL for ${dbType}:
            "${naturalQuery}"
            ...
            4. Ensure the query is optimized for ${dbType}
      ...
      logger.error(`SQL generation failed for ${dbType}:`, error);
      throw error;
    }
  }
